...
//                    send image
                    ...
                    message.setChatId(chatId);
                    attachButtons(message, Map.of(
                        "Купити Джавелін (50 монет)", "buy_javeline"
                ));

                    sendApiMethodAsync(message);
                }
                if (update.getCallbackQuery().getData().equals("buy_javeline") && getLevel(chatId) == 4) {
                    setLevel(chatId, 5);
//                    send image
                    sendImage("final", chatId);
                    SendMessage message = createMassage("*Джавелін твій. Повний вперед!*");
                    message.setChatId(chatId);
                    sendApiMethodAsync(message);
                }
